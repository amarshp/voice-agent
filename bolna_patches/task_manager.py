...
class TaskManager(BaseManager):
    ...
    async def __process_http_transcription(self, message):
        # FIX: http path forwarded interim dicts / "speech_started" to the string-only
        # _handle_transcriber_output -> ".strip() on dict" crash. Forward only FINAL
        # transcripts (extract content whether dict or str); skip interim/events.
        _d = message.get("data")
        if isinstance(_d, dict):
            if _d.get("type") != "transcript":
                return
            _d = _d.get("content") or ""
        if not isinstance(_d, str):
            return
        _d = _d.strip()
        if not _d or _d == "speech_started":
            return
        message = dict(message); message["data"] = _d
    ...
    async def run(self):
        ...
        try:
            ...
            import traceback as _tb; logger.info("Exception occurred " + str(e)); logger.info("FULLTRACE " + _tb.format_exc().replace(chr(10)," || "))
            raise
