...
# --- inbound TwiML for Twilio Media Streams (dynamic: agent_id via query) ---
from fastapi import Request as _Req
from fastapi.responses import Response as _XMLResponse


@app.api_route("/voice", methods=["GET", "POST"])
async def voice_inbound(request: _Req, agent_id: str = Query(...)):
    host = request.headers.get("x-forwarded-host") or request.url.hostname
    wss = f"wss://{host}/chat/v1/{agent_id}"
    twiml = (
    ...
    return _XMLResponse(content=twiml, media_type="text/xml")
